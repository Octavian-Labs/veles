...
class Парсер:
    ...
    def это(self, знач):
        """Текущий токен — оператор или слово с данным значением.

        СТР/ЧИСЛО не считаются операторами: строка "-" не унарный минус."""
        t = self.ток()
        return t.вид in ("ОП", "ИМЯ") and t.знач == знач

    def это_оп(self, *значи):
        """Текущий токен — оператор (ОП) из списка значений."""
        t = self.ток()
        return t.вид == "ОП" and t.знач in значи
    ...
    def _параметры(self) -> list:
        ...
        while not self.это(")"):
            ...
            while self.это(","):
                ...
                if nxt.вид == "ИМЯ" and nxt2.вид == "ОП" and nxt2.знач == ",":
                    self.далее()
                    имена.append(self.жди_имя())
                    continue
                if nxt.вид == "ИМЯ" and (
                    nxt2.вид == "ИМЯ" or (nxt2.вид == "ОП" and nxt2.знач in ("*", "["))
                ):
                    self.далее()
                    имена.append(self.жди_имя())
                    continue
    ...
    def оператор(self):
        ...
        if t.вид == "ИМЯ" and self.ток(1).вид == "ОП" and self.ток(1).знач == ":=":
            self.далее()
            self.далее()
    ...
    def _тип_после_имени(self) -> bool:
        ...
        if t.вид == "ИМЯ" and t.знач in _ТИПЫ:
            return True
        if (t.вид == "ОП" and t.знач == "*") or (t.вид == "ИМЯ" and t.знач == "функ"):
            return True
        if t.вид == "ОП" and t.знач == "[":
            # «имя []Т» — срез-тип; «имя [выр]» — индексация, не объявление
            n2 = self.ток(2)
            return n2.вид == "ОП" and n2.знач == "]"
        if t.вид == "ИМЯ":
            n2 = self.ток(2)
            return (n2.вид == "ОП" and n2.знач in ("=", "[")) or n2.вид == "НС"
        return False
    ...
    def _если(self, line):
        ...
        while True:
            ...
            if (
                self.это("а")
                and self.ток(1).вид == "ИМЯ"
                and self.ток(1).знач == "если"
            ):
                self.далее()
                self.далее()
    ...
    def _сравн(self):
        ...
        while True:
            t = self.ток()
            if t.вид == "ОП" and t.знач in _СРАВ:
                оп = self.далее().знач
                e = N.Бин(оп, e, self._бит_или(), t.строка)
                continue
            if (t.вид == "ИМЯ" and t.знач == "в") or (
                t.вид == "ИМЯ"
                and t.знач == "не"
                and self.ток(1).вид == "ИМЯ"
                and self.ток(1).знач == "в"
            ):
                line = t.строка
                neg = t.знач == "не"
                self.далее()
